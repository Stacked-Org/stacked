library auto_route;

export 'package:meta/meta.dart' show required;

export 'legacy.dart' hide RouteData, RouteDef;
export 'src/stacked_route_guard.dart';
export 'src/common/common.dart';
export 'src/route/page_route_info.dart';
export 'src/route/route_args.dart';
export 'src/route/route_data.dart';
export 'src/route/route_def.dart';
export 'src/router/stacked_route_page.dart';
export 'src/router/stacked_router_config.dart';
export 'src/router/stacked_router_x.dart';
export 'src/router/controller/routing_controller.dart';
export 'src/router/widgets/stacked_router.dart';
export 'src/router/widgets/stacked_router_delegate.dart';

// export 'auto_route_annotations.dart';
